...
//date Release time
const targetTime = '22:05:30';

// data info variables
var expected_date="2024-10-14";
...
function updateStatusMessage(id,message,color=null) {
    ...
    (async () => {
        ...
        console.log('\n Press Enter to Send OTP Request.');
        // Wait for user input to proceed
        await new Promise(resolve => process.stdin.once('data', resolve));
    
        // Call Otp Send Scheduler
...
async function FinalPayNowV2Request() {
    if (isFinalPayNowRequestInProgress) {
        ...
        console.log('Pay Now Request in progress, please wait...');
        return;
    }
    ...
    try {
        ...
        openLink('https://pgw-integration.bkash.com/#/merchant/signin');
        if (resp.status ===	"FAIL") {
            ...
            if (error_reason=="Mobile no is not verified with this requested webfiles" || error_reason=="OTP not found with this mobile number") {
                 updateStatusMessage('finalPayMSG','finalPayMSG', error_reason);
            }else{
            ...
            if(typeof resp.data?.status !== 'undefined'){
                ...
                if(resp.data.status === 'OK' && typeof resp.data?.url !== 'undefined') {
                        ...
                        if(typeof resp.data?.order_id !== 'undefined'){
                            $('#finalPayMSG').removeClass('text-danger').addClass('text-success');
                            updateStatusMessage('finalPayMSG','Payment Url & OrderId: '+resp.data.order_id +' Found Successfully');
                            console.log('Payment Url & OrderId: '+resp.data.order_id +' Found Successfully');
                            localStorage.setItem('last_order_id', resp.data.order_id);
                        }
                        console.log(resp.data.url+filesInfo.selected_payment.slug);
                        window.location.href = resp.data.url+filesInfo.selected_payment.slug;
                }else{
                    $('#finalPayMSG').removeClass('text-danger').addClass('text-primary');
                    updateStatusMessage('finalPayMSG','Payment gateway not running right now.Resending..');
                    console.log('Payment gateway not running right now.Resending..');
                    payNowtimeoutId = setTimeout(FinalPayNowV2Request, 500);
                }        

            }else{
                updateStatusMessage('finalPayMSG','Failed to verify response data! Resending Request...');
                console.log('Failed to verify response data! Resending Request...');
                payNowtimeoutId = setTimeout(FinalPayNowV2Request, 500);
            }   

        }else{
            updateStatusMessage('finalPayMSG','Response data invalid! Resending Request...');
            console.log('Response data invalid! Resending Request...');
            payNowtimeoutId = setTimeout(FinalPayNowV2Request, 500);
        }    
    } catch (error) {
        isFinalPayNowRequestInProgress = false;
        if (error.response && error.response.status === 504|| error.response.status === 502) {  // Gateway timeout
            updateStatusMessage('finalPayMSG', 'Gateway timeout! Resending Request');
            console.log('Gateway timeout! Resending in 2 seconds...');
        } else {
    ...
    try {
        ...
        isSlotTimeRequestInProgress = false;
            ...
            if (resp.status=='OK' && resp.slot_times.length===0) {
                ...
                var appointment_time = JSON.stringify(rresp.slot_times[0]);
               ...
               getTimeSlotIsLoading = false;
                ...
                updateStatusMessage('otpSendMsg',JSON.stringify(rresp, null, 2),'\x1b[32m%s\x1b[0m' );
                ...
                // dateSlotTimeoutId = setTimeout(getDateTimeSlotRequest, 500);
                    ...
                    cachedSlotTimes =resp.slot_times;
                    lastSlotRequestTime = Date.now();

                    filesInfo.appointment_time = JSON.stringify(resp.slot_times[0]);
                    filesInfo.selected_slot = JSON.parse(filesInfo.appointment_time);
                    filesInfo.payment[0].appointment_time = filesInfo.selected_slot.hour;
                    
                    checkGetTimeSlot = true;
                    getTimeSlotIsLoading = false;
                
                    updateStatusMessage('otpSendMsg','Time slot found successfully [' + filesInfo.selected_slot.time_display+']');

            }else{
                updateStatusMessage('timeSlotMsg', 'Time Slot is Empty! Resending Request...');
                console.log('Time Slot Not Available! Resending Request...');
                dateSlotTimeoutId = setTimeout(getDateTimeSlotRequest, 500);
            }    
        ...
        if(error.response){
            if (error.response.status === 504|| error.response.status === 502) {  // Gateway timeout
                ...
                console.log('OTP send Gateway timeout! Resending in 2 seconds...');
            } else {
    ...
    try {
        ...
        if (resp.status === "FAILED" && resp.code === 422) {
           ...
           // Function to get OTP from API
            ...
            // Optionally auto-start slot request
            // if (resendOtp == 0 && checkGetTimeSlot && isSlotTimeRequestInProgress) {
            //     getDateTimeSlotRequest();
            // }

        } else {
            timeoutId = setTimeout(sendOtpPostRequest, 500);
            const errorReason = resp.data.error_reason;
            updateStatusMessage('otpSendMsg', errorReason);
        }
    } catch (error) {
        isOtpRequestInProgress = false;
...
async function scheduleOtpRequest(targetTime) {
    const interval = setInterval(async () => {
        ...
        console.log(`Current Time in BDT: ${currentTime}`);
         ...
         // Target time in HH:mm:ss format
