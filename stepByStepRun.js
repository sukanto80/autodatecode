...
//Date Release time
const targetTime = '08:21:40'; // Target time in HH:mm:ss format
...
function getBangladeshTime() {
 ...
 function isGreaterThan5Minutes(previousTime) {
    // Get the current time in the specified timezone (Bangladesh)
    const options = { timeZone: 'Asia/Dhaka' };
    const currentTime = new Date().toLocaleString('en-US', options);

    // Convert previousTime (HH:mm:ss) to a Date object
    const previousTimeArray = previousTime.split(':');
    const currentBangladeshDate = new Date().toLocaleDateString('en-US', { timeZone: 'Asia/Dhaka' });

    const previousDateTime = new Date(`${currentBangladeshDate} ${previousTime}`);
    const previousTimeMillis = previousDateTime.getTime();
    const currentMillis = new Date(currentTime).getTime();

    // Calculate the time difference in milliseconds
    const timeDifference = currentMillis - previousTimeMillis;

    // Return true if the time difference is greater than 5 minutes, else false
    ...
    try {
        ...
        if (resp.status ===	"FAIL") {
            ...
            if (error_reason=="Mobile no is not verified with this requested webfiles" || error_reason=="OTP not found with this mobile number" ||  error_reason=="OTP expired. Please try again" || error_reason=="OTP does not match. Please try again") {
                ...
                if(ReceivedOTP.length == 6 && isGreaterThan5Minutes(OtpSendingTime)){
                    updateStatusMessage('finalPayMSG',error_reason+'.Resending...');
...
async function sendVerifyOtpRequest() {
    ...
    try {
        ...
        if (resp.status ===	"FAILED") {
            ...
            }else if(error_reason=='Slot is not available' && isGreaterThan5Minutes(OtpSendingTime)){
            ...
            updateStatusMessage('otpVerifyMsg',JSON.stringify(resp, null, 2),'\x1b[32m%s\x1b[0m' );
                ...
                if (expected_date.length!==0) {
                    ...
                    updateStatusMessage('otpVerifyMsg','Response from fetched slots: '+ DateSlots[0]);
                    getDateTimeSlotRequest();
                }
    ...
    try {
        ...
        if (resp.status === "FAILED" && resp.code === 422) {
           ...
           timeoutId = setTimeout(sendOtpPostRequest, 300);
            ...
            OtpSendingTime=getBangladeshTime();
